...
public class ItemWormholeLinker extends Item {
    ...
    public ItemWormholeLinker() {
        setRegistryName(RFGen.MODID, "wormhole_linker");
        setUnlocalizedName(RFGen.MODID + ".wormhole_linker");
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (player.isSneaking()) {
            if (!world.isRemote && stack.hasTagCompound() && hasStoredLink(stack.getTagCompound())) {
                clearStored(stack);
                player.sendStatusMessage(new TextComponentTranslation("message.rfgen.linker.cleared"), true);
            } else if (!world.isRemote) {
                clearStored(stack);
                player.sendStatusMessage(new TextComponentTranslation("message.rfgen.linker.empty"), true);
            }
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand,
        ...
        if (player.isSneaking()) {
            if (!world.isRemote) {
                ...
                player.sendStatusMessage(new TextComponentTranslation(
                        had ? "message.rfgen.linker.cleared" : "message.rfgen.linker.empty"), true);
            }
        ...
        if (!hasStoredLink(nbt)) {
            ...
            player.sendStatusMessage(new TextComponentTranslation(
                    "message.rfgen.linker.stored", pos.getX(), pos.getY(), pos.getZ()), true);
            return EnumActionResult.SUCCESS;
        }
